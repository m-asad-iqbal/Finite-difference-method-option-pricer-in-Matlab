%Crank-Nicholson Scheme to solve Black Scholes PDE with up and out call option
clear all, close all
global T alpha
%% Parameters
K = 90; % Strike Price
...
for k=1:M+1
    ...
    % lower, main and upper diagonal of the tridiagonal matrix for the
    % explicit part of the numerical scheme
    l_n=(0.5*(dtau/(dS^2))*(sigma(tau(k),S(:)).^2).*(S(:).^2)-0.5*(dtau/dS)*(r-q).*S(:));
    d_n=(2-(dtau/(dS^2))*(sigma(tau(k),S(:)).^2).*(S(:).^2)-r*dtau);
    u_n=(0.5*(dtau/(dS^2))*(sigma(tau(k),S(:)).^2).*(S(:).^2)+0.5*(dtau/dS)*(r-q).*S(:));

    % Constructio of the the vector b in the linear problem b in the
    % Crank-Nicholson numerical scheme
    B_x=spdiags([l_n d_n u_n],-1:1,N-1,N-1);
    B=B_x*V(:,k);
    
    % lower, main and upper diagonal of the tridiagonal matrix for the
    % implicit part of the numerical scheme
    l_n1=(-0.5*(dtau/(dS^2))*(sigma(tau(k+1),S(:)).^2).*(S(:).^2)+0.5*(dtau/dS)*(r-q).*S(:));
    d_n1=(2+(dtau/(dS^2))*(sigma(tau(k+1),S(:)).^2).*(S(:).^2)+r*dtau);
    u_n1=-(0.5*(dtau/(dS^2))*(sigma(tau(k+1),S(:)).^2).*(S(:).^2) +0.5*(dtau/dS)*(r-q).*S(:));
    

    V(:,k+1)=tridiag(d_n1,u_n1,l_n1,B);
end
toc
%% Results
